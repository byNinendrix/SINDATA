...
import { AnimatedOwlLogo, type OwlLogoState } from './AnimatedOwlLogo';

export interface AuthOutletContext {
  setOwlLogoState: (state: OwlLogoState) => void;
}

export function AuthLayout() {
  const [owlLogoState, setOwlLogoState] = useState<OwlLogoState>('trackingMouse');
        ...
        <section className="hidden w-full flex-col justify-between bg-sindata-900 px-10 py-12 text-slate-100 lg:flex lg:w-1/2">
          <div>
            <div className="flex items-center gap-3">
              <AnimatedOwlLogo state={owlLogoState} className="h-9 w-9 rounded-md ring-1 ring-cyan-200/40" />
