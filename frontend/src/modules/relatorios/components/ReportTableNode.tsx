...
export const ReportTableNode = memo(function ReportTableNode({
  table,
  position,
  selectedFieldKeys,
  ...
  return (
    <div
      className="absolute z-[3] flex max-h-[420px] w-[320px] flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-md"
      style={{ left: position.x, top: position.y }}
      onPointerDown={(event) => event.stopPropagation()}
    >
      <div
        className="mb-2 flex cursor-move items-start justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 transition hover:border-cyan-200"
        onPointerDown={(event) => onStartTableDrag(table.id, event)}
      >
        <div>
          <h4 className="text-sm font-semibold text-slate-900">{table.name}</h4>
          <p className="text-[11px] text-slate-500">{selectedCount} campo(s) no resultado</p>
        </div>
      ...
      <div className="mb-2 rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] text-slate-600">
        Checkbox: exibir no relatorio | Conexao: arrastar para criar ligacao
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto pr-1" onScroll={onFieldsScroll}>
        {previewFields.map((field) => {
          ...
          const isSelectable = field.isSelectable !== false;
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 rounded-md border px-1.5 py-1 text-xs text-slate-700 transition ${
                isDropTarget
            ...
            >
              <label className="flex min-w-0 flex-1 items-center gap-2">
                <input
                  ...
                  title={isSelectable ? 'Marcar campo para aparecer no relatorio' : 'Campo nao disponivel para retorno'}
                />
              ...
              <button
                ...
                title="Arraste para criar uma ligacao com outro campo"
              >
                <Link2 size={12} />
              </button>
            </div>
